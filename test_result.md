...
agent_communication:
    ...
    -agent: "main"
    -message: "Multi-Source Event Modal + Frontend Analytics Wiring — both shipped. (1) VERIFIED VIA SCREENSHOT TOOL: Multi-source events from backend AI dedup now render correctly. Tested on the June 3 multi-source event 'כנס הריון ולידה' which was deduplicated from eilat_muni + smarticket. EventCard shows the new badge '🏛️ 2 מקורות' (gov icon + count), tap opens a bottom-sheet modal titled 'אירוע זה פורסם ב-2 מקורות' that lists both sources, each as a tappable row with its URL. Government source (eilat_muni → עיריית אילת) is correctly styled with teal background + 🏛️ icon per Play policy. Sources sorted gov-first. Close button works. (2) Wired analytics in /app/frontend/utils/analytics.ts: screen_view fires on mount in every tab (home/news/jobs/businesses/eilatush), business_view on /business/[id], job_view on /job/[id], news_view on /article/[id]. Also wired business_phone_click (detail card + sticky CTA + list-card icon), business_directions_click (address tap), business_website_click (website tap), job_outbound_click (open-source button), news_outbound_click (article source pill + read-at-source button). Verified in DB: analytics_events collection now contains screen_view{screen:'home'}, business_view, event_outbound_click events. Cleanup: removed unused analytics imports from components.tsx."
...
agent_communication:
    ...
    -agent: "main"
    -message: "Eilatush Persona Polish (UI + voice): (1) Removed all fish emoji 🐠 from chat UI, welcome messages, backend LLM prompts, and fallback replies. (2) Rewrote LLM system prompt + system_message for the second-pass reply generator to enforce **feminine Hebrew grammar** throughout — Eilatush is now a woman who always speaks of herself with feminine verbs ('אני יודעת', 'מצאתי לך', 'חשבתי'). Added explicit rule #8/#9 banning masculine voice and fish emoji. (3) Removed duplicate 'הזמן חבר' + 'צור קשר' CTAs from the welcome message body — those actions remain accessible in the header icon row (no duplication). (4) Replaced the sparkles-in-teal-circle mascot with the actual `eilatush-mascot.png` asset in both the chat header (42×42) and beside each bot bubble (28×28), matching the tab-bar icon. New peach-colored background (#FFE8D6) circle wraps the image. Files touched: /app/frontend/app/(tabs)/eilatush.tsx, /app/backend/server.py (EILATUSH_REPLY_PROMPT + _generate_reply system_message + fallback text)."
    -agent: "main"
    -message: "Eilatush Addressee Gender (one-time onboarding + persistent per-device): (1) On first load, instead of the generic welcome, Eilatush now greets with 'שמחה להכיר! לפני שנתחיל - איך מתאים לך שאפנה?' plus two chips 👨 בלשון זכר / 👩 בלשון נקבה. (2) User's choice persists to AsyncStorage under key `eilatush_user_gender_v1` and is sent with every chat request as `user_gender: 'm' | 'f'`. (3) Backend `ChatRequest` now accepts `user_gender` and `_generate_reply` injects a per-turn gender directive into both the Claude `system_message` and the user prompt — so replies address the user in the correct Hebrew form ('אתה יכול' vs 'את יכולה', 'מצאתי לך' in matching verb form). (4) Welcome text also adapts: 'שאל אותי' (male) vs 'שאלי אותי' (female). (5) 'New chat' button now preserves gender preference (only wipes messages + session), so users set it once and Eilatush remembers forever. Files: /app/frontend/app/(tabs)/eilatush.tsx, /app/frontend/api.ts, /app/backend/server.py. Verified: web test confirmed male-selected user got back 'אם אתה יותר לסרט... מה מושך אותך?' in masculine form; preference survived full page reload and 'שיחה חדשה' reset."
